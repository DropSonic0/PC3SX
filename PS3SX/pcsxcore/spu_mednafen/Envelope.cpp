...
void						PSX::SPU::Envelope::Freeze					(Envelope_State* aState, bool aLoad)
{
	if(aLoad)
	{
		State = aState->State;
		AttackModeExp = aState->AttackModeExp;
		AttackRate = aState->AttackRate;
		DecayRate = aState->DecayRate;
		SustainLevel = aState->SustainLevel;
		SustainModeExp = aState->SustainModeExp;
		SustainIncrease = aState->SustainIncrease;
		SustainRate = aState->SustainRate;
		ReleaseModeExp = aState->ReleaseModeExp;
		ReleaseRate = aState->ReleaseRate;
		EnvelopeVol = aState->EnvelopeVol;
		EnvelopeVol_f = aState->EnvelopeVol_f;
		lVolume = aState->lVolume;
	}
	else
	{
		aState->State = State;
		aState->AttackModeExp = AttackModeExp;
		aState->AttackRate = AttackRate;
		aState->DecayRate = DecayRate;
		aState->SustainLevel = SustainLevel;
		aState->SustainModeExp = SustainModeExp;
		aState->SustainIncrease = SustainIncrease;
		aState->SustainRate = SustainRate;
		aState->ReleaseModeExp = ReleaseModeExp;
		aState->ReleaseRate = ReleaseRate;
		aState->EnvelopeVol = EnvelopeVol;
		aState->EnvelopeVol_f = EnvelopeVol_f;
		aState->lVolume = lVolume;
	}
}
