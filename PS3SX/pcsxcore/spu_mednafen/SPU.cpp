...
void						PSX::SPU::SPU::Freeze				(SPU_State* aState, bool aLoad)
{
	if(aLoad)
	{
		CycleCounter = aState->CycleCounter;
		StreamIndex = aState->StreamIndex;
		Control = aState->Control;
		Status = aState->Status;
		Address = aState->Address;
		IRQAddress = aState->IRQAddress;
		IRQHit = aState->IRQHit != 0;
		memcpy(RegisterArea, aState->RegisterArea, sizeof(RegisterArea));
		memcpy(Memory, aState->Memory, sizeof(Memory));
	}
	else
	{
		aState->CycleCounter = CycleCounter;
		aState->StreamIndex = StreamIndex;
		aState->Control = Control;
		aState->Status = Status;
		aState->Address = Address;
		aState->IRQAddress = IRQAddress;
		aState->IRQHit = IRQHit ? 1 : 0;
		memcpy(aState->RegisterArea, RegisterArea, sizeof(RegisterArea));
		memcpy(aState->Memory, Memory, sizeof(Memory));
	}

	for(int i = 0; i < 24; i++)
		Channels[i]->Freeze(&aState->Channels[i], aLoad);

	Streamer.Freeze(&aState->Streamer, aLoad);
	Noise.Freeze(&aState->Noise, aLoad);
}
