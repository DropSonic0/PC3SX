...
void				PSX::SPU::SampleStream::Freeze					(SampleStream_State* aState, bool aLoad)
{
	if(aLoad)
	{
		Start = aState->Start;
		Current = aState->Current;
		Loop = aState->Loop;
		Index = aState->Index;
		for(int i = 0; i < 28; i++)
			DecodeBuffer[i] = aState->DecodeBuffer[i];
		for(int i = 0; i < 2; i++)
			PreviousSamples[i] = aState->PreviousSamples[i];
	}
	else
	{
		aState->Start = Start;
		aState->Current = Current;
		aState->Loop = Loop;
		aState->Index = Index;
		for(int i = 0; i < 28; i++)
			aState->DecodeBuffer[i] = DecodeBuffer[i];
		for(int i = 0; i < 2; i++)
			aState->PreviousSamples[i] = PreviousSamples[i];
	}
}
...
void				PSX::SPU::Channel::Freeze					(Channel_State* aState, bool aLoad)
{
	if(aLoad)
	{
		DecodeStream = aState->DecodeStream;
		HasDecodeBuffer = aState->HasDecodeBuffer != 0;
		SamplePosition = aState->SamplePosition;
		SampleIncrement = aState->SampleIncrement;
		bReverb = aState->bReverb;
		bRVBActive = aState->bRVBActive;
		bNoise = aState->bNoise;
		bFMod = aState->bFMod;
		RawPitch = aState->RawPitch;
		Volume[0] = aState->Volume[0];
		Volume[1] = aState->Volume[1];
	}
	else
	{
		aState->DecodeStream = DecodeStream;
		aState->HasDecodeBuffer = HasDecodeBuffer ? 1 : 0;
		aState->SamplePosition = SamplePosition;
		aState->SampleIncrement = SampleIncrement;
		aState->bReverb = bReverb;
		aState->bRVBActive = bRVBActive;
		aState->bNoise = bNoise;
		aState->bFMod = bFMod;
		aState->RawPitch = RawPitch;
		aState->Volume[0] = Volume[0];
		aState->Volume[1] = Volume[1];
	}

	ADSR.Freeze(&aState->ADSR, aLoad);
	Sample.Freeze(&aState->Sample, aLoad);
	Filter.Freeze(&aState->Filter, aLoad);
}
